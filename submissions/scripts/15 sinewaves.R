...
waves |>
  ...
  theme_void()+
  geom_point(data = sky, color = sky$color, position = position_jitter(width = .4, height = .6), size = sample(seq(.1,10, length.out = 25), nrow(sky), replace = TRUE))+
  geom_path(color = waves$color, linewidth = 2, alpha = .8)+
  geom_path(color = rev(waves$color), linewidth = .1, alpha = .2)+
  geom_point(data = cloud_box, size = sample(seq(5,10, length.out = 30), nrow(cloud_box), replace = TRUE), alpha = .01, position = position_jitter(width = 5, height = .1), color = sample(grays)) +
  geom_point(data = texture, size = sample(seq(10,20, length.out = 50), nrow(texture), replace = TRUE), alpha = .002, color = "#ffffff", position = position_jitter(width = .5, height = .4))+
