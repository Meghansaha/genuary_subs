...
font_add(family = "tequila", regular = "submissions/tequila/TEQUILA_.TTF")
showtext_auto()
...
texture <- tibble(crossing(y = -10:20,
                           x = 0:48))
...
boxes |>
  ...
  geom_point(data = texture, aes(x,y), inherit.aes = FALSE, 
             size = sample(1:50, nrow(texture), replace = TRUE), 
             alpha = .04, color = "#F2EDC5", 
             position = position_jitter(width = .9, height = .5))+
  geom_text(data = words, aes(x,y, label = text), inherit.aes = FALSE, 
            family = "tequila", 
  ...
  geom_path(data = texture, aes(x,y), inherit.aes = FALSE,  alpha = .04, size = 10, color = "#751109", position = position_jitter(width = .9, height = .5))+
  geom_segment(size = boxes$size) +
  geom_point(data = chord_points, aes(x,y), size = 3, inherit.aes = FALSE, shape = 21, color = "#000000", fill = "#751109")+
  geom_text(data = chord_annotations, aes(x,y, label = chords), inherit.aes = FALSE, family = "tequila", size = 12, color = "#751109", fontface = "bold")+
  geom_text(data = xes, aes(x,y, label = text), inherit.aes = FALSE, family = "tequila", size = 6 )+

  coord_equal(ylim = c(-10,20), xlim = c(0,48))
